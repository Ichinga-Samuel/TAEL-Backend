...
router.post('/login',  async (req, res) => {
    try{
        ...
        let user = await User.findOne({email: req.body.email}).select(fields).populate({path: 'fave', select: {id: 1, title: 1, imageUrl: 1}}).populate('reviews');
        if(user.isValidPassword(req.body.password)){
            ...
            res.status(200).json({msg: 'Invalid Credentials', status: false})
        }
